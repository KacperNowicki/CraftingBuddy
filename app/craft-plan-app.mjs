...
server.on("error", (error) => {
  if (error?.code === "EADDRINUSE") {
    console.log(`CraftPlan app is already running at ${APP_URL}`);
    if (process.env.CRAFTINGBUDDY_NO_OPEN !== "1") openUrl(APP_URL);
    process.exit(0);
  }
...
server.listen(DEFAULT_PORT, "127.0.0.1", () => {
  console.log(`CraftPlan app running at ${APP_URL}`);
  if (process.env.CRAFTINGBUDDY_NO_OPEN !== "1") openUrl(APP_URL);
});
